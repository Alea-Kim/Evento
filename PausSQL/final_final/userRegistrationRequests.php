...
<head>
	<meta charset="utf-8">
	<title>A M A L G A M A T E | User Registration Requests</title>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="wow bounceIn">
						<h2 class="wow bounceIn"> USER APPROVAL</h2>
						<hr>
					</div>
					<div class="iso-section wow fadeIn" data-wow-delay="0.6s">						
						<ul class="filter-wrapper clearfix">
							<?php
								$query = "select * from user_account where is_approved='f'";
								$result = pg_query($query);
								...
								while($myrow = pg_fetch_assoc($result)) {
									...
									printf ('<li><a>%s | %s | %s | %s</a></li>', $myrow['username'], $myrow['password'], $myrow['name'], $myrow['email']);
									echo '<li> <button value="' . $Username . '" class="approve button btn-success"> APPROVE </button> <button value="' . $Username . '" class="disapprove button btn-danger"> DISAPPROVE </button> </li> <br />';
								}
	...
	<script>
		$(document).ready(function(){
			$(".approve").click(function(){
				$.post("http://localhost/final_final/approve.php",{
					username: $(this).val()
				},
				function (res){
			...
			$(".disapprove").click(function(){
				$.post("http://localhost/final_final/disapprove.php",{
					username: $(this).val()
				},
				function (res){
					console.log($(this).val());
